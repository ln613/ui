...
export const ElemDiv = p => <div className={'pv8 ' + p.class}>{p.children}</div>
...
const checkBox = p => <input type="checkbox" checked={p.value} {...p} />
...
const checkBoxWithLabel = p => (
  <ElemDiv>
    {checkBox(p)}
    {p.title ? (
      <label htmlFor={p.name} className="label-align">
        {p.title}
      </label>
    ) : null}
  </ElemDiv>
